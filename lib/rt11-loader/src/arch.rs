...
//!
//! This module encapsulates all architecture specifics of the dynamic loader.
//! Every architecture specific bit of the loader is extracted into a macro,
//! function, etc., that is then implemented for each supported architecture.
//!
...
// Target Architecture
//
// We need architecture-dependent assembly to implement entry-points and
// invoke system calls. To avoid spurious linker errors in dependent
// crates, we check for supported architectures here and error out right
// away.
#[cfg(not(any(
    doc,
    target_arch = "arm",
...
pub mod doc {
    ...
    /// Loader Entry-point Code
    ///
    /// This macro expands to the inline-assembly of the code used to
    /// implement the entry-point of the loader. It can use `{0}` to refer
