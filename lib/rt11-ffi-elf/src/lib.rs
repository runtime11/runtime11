...
//! environments.
//!
//! The main definitions are provided in the `elf` module. It contains
//! generic type definitions suitable for 32bit and 64bit targets. The `elf32`
//! and `elf64` module contain suitable type-aliases for the respective
//! environments. Lastly, the `elfn` module is an alias for the native module.
//!
//! Unless explicitly noted, constants are the same on all architectures and
//! can be used from either of the modules.
...
/// Generic Utilities
///
/// The utility module contains all generic utilities required by the main
/// module. These are completely decoupled and can be used independently, or
/// extracted into separate crates.
pub mod util {
    /// An 8bit aligned ZST
    ///
    /// This type can be used to align structures to at least 8bit by
    ...
    /// A 16bit aligned ZST
    ///
    /// This type can be used to align structures to at least 16bit by
    /// embedding it in the structure. It works similar to other phantom-marker
    /// types.
    #[repr(C, align(2))]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    ...
    /// A 32bit aligned ZST
    ///
    /// This type can be used to align structures to at least 32bit by
    /// embedding it in the structure. It works similar to other phantom-marker
    /// types.
    #[repr(C, align(4))]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    ...
    /// A 64bit aligned ZST
    ///
    /// This type can be used to align structures to at least 64bit by
    /// embedding it in the structure. It works similar to other phantom-marker
    /// types.
    #[repr(C, align(8))]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
///
/// The `elf` module exports all relevant definitions of the ELF format. It
/// is recommended to consult an ELF manual for specific context on the
/// individual definitions.
///
/// Deviations from the reference implementation in C are noted on each
/// definition.
///
/// Note
/// ----
...
/// lost support over the years and explicitly sized integers are preferred.
...
pub mod elf {
    ...
    /// Typically, it represented as a 16-byte array with open-coded byte
    ...
    /// ELF Header
    ///
    /// The first bytes of an ELF file are occupied by the ELF header, which
    /// itself starts with the ELF identification table.
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Ehdr<SIZE, ALIGN> {
        pub _align: ALIGN,
        pub e_ident: Ident,
        pub e_type: u16,
        pub e_machine: u16,
    ...
    /// Section Header
    ///
    /// A section header describes a section of an ELF file. It contains all
    /// metadata on the section and points to the relevant parts in the file
    /// for section content.
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Shdr<SIZE, ALIGN> {
        pub _align: ALIGN,
        pub sh_name: u32,
        pub sh_type: u32,
        pub sh_flags: SIZE,
    ...
    /// metadata relevant to a segment and points to the segment data in the
    /// file.
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Phdr<SIZE, ALIGN> {
        pub _align: ALIGN,
        pub p_type: u32,
        pub p_offset: u32,
        pub p_vaddr: SIZE,
    ...
    /// ----
    ///
    /// Traditionally, the struct has a `d_un` field, which itself is a union
    /// with two equally sized fields `d_val` and `d_ptr`. Since they have the
    /// same size and evaluate to the same integer type, they are merely
    /// aliases.
    ///
    /// This Rust struct uses `d_val` directly and does not provide any alias.
    /// This makes the struct a lot easier to work with in Rust, since a union
    /// would require a lot of manual trait implementations and unsafe
    /// operations, which seems excessive given that it is a mere alias.
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Dyn<SIZE, ALIGN> {
        pub _align: ALIGN,
        pub d_tag: SIZE,
        pub d_val: SIZE,
    }
    ...
    ///
    /// Code relocations with implicit addend use this structure to describe
    /// all relevant metadata for the relocation.
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Rel<SIZE, ALIGN> {
        pub _align: ALIGN,
        pub r_offset: SIZE,
        pub r_info: SIZE,
    }
    ...
    #[repr(C)]
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
    pub struct Rela<SIZE, ALIGN, ADDEND> {
        pub _align: ALIGN,
        pub r_offset: SIZE,
        pub r_info: SIZE,
        pub r_addend: ADDEND,
    ...
    impl Ident {
        ...
        pub const ELFOSABI_SYSV: u8 = Self::ELFOSABI_NONE;
        pub const ELFOSABI_HPUX: u8 = 1;
        pub const ELFOSABI_NETBSD: u8 = 2;
        pub const ELFOSABI_GNU: u8 = 3;
        pub const ELFOSABI_LINUX: u8 = Self::ELFOSABI_GNU;
        pub const ELFOSABI_HURD: u8 = 4;
        pub const ELFOSABI_86OPEN: u8 = 5;
        pub const ELFOSABI_SOLARIS: u8 = 6;
        pub const ELFOSABI_AIX: u8 = 7;
        pub const ELFOSABI_MONTEREY: u8 = Self::ELFOSABI_AIX;
    ...
    impl<SIZE, ALIGN> Ehdr<SIZE, ALIGN> {
        ...
        pub const EM_486: u16 = Self::EM_IAMCU; // from: linux
        pub const EM_860: u16 = 7;
        pub const EM_MIPS: u16 = 8;
        pub const EM_S370: u16 = 9;
        pub const EM_MIPS_RS3_LE: u16 = 10;
        pub const EM_MIPS_RS4_BE: u16 = Self::EM_MIPS_RS3_LE; // from: linux
        ...
        pub const EM_ALPHA_STD: u16 = Self::EM_ALPHA; // from: linux
        ...
        pub const EM_AMD64: u16 = Self::EM_X86_64; // from: bionic
        ...
        pub const EM_CYGNUS_FRV_INTERIM: u16 = Self::EM_FRV_INTERIM; // from: uclibc-ng
...
/// ELF for 32bit
///
/// This module exports all types of the generic module but typed for 32bit
...
/// ELF for 64bit
///
/// This module exports all types of the generic module but typed for 64bit
/// machines. That is, these types follow the 64bit ELF conventions and use
/// 64bit addresses and offsets.
///
/// Note that some types of this module deviate structurally from the
...
mod tests {
    ...
    #[test]
    fn test_util_align() {
        assert_eq!(align_of::<util::PhantomAlign8>(), 1);
        assert_eq!(align_of::<util::PhantomAlign16>(), 2);
        assert_eq!(align_of::<util::PhantomAlign32>(), 4);
        assert_eq!(align_of::<util::PhantomAlign64>(), 8);
        assert_eq!(size_of::<util::PhantomAlign8>(), 0);
        assert_eq!(size_of::<util::PhantomAlign16>(), 0);
        assert_eq!(size_of::<util::PhantomAlign32>(), 0);
        assert_eq!(size_of::<util::PhantomAlign64>(), 0);
    }

    #[test]
    fn test_elf_typeinfo() {
        assert_eq!(align_of::<elf::Ident>(), 1);
        assert_eq!(size_of::<elf::Ident>(), 16);
        ...
        assert_eq!(align_of::<elfn::Addend>(), align_of::<isize>());
        assert_eq!(size_of::<elfn::Addend>(), size_of::<isize>());
        assert_eq!(align_of::<elfn::Align>(), align_of::<usize>());
        assert_eq!(size_of::<elfn::Align>(), 0);
        assert_eq!(align_of::<elfn::Size>(), align_of::<usize>());
        assert_eq!(size_of::<elfn::Size>(), size_of::<usize>());
